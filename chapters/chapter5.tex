...
本章用五次多项式插值作为图\ref{fig.control.motor}的轨迹发生器中的轨迹规划算法，可以做到机器人在关节空间的加速度连续且可导，其导数加加速度（冲击）连续，其轨迹比较平滑\upcite{LiuPhD2009}，一般机器人控制器的关节运动都是采用五次多项式插补。在仿真过程中，可以借助于机器人相关工具箱里提供的函数完成插值规划部分。
