...
图\ref{fig.robot}的机器人具有六个自由度，分别由六个关节J1~J2的执行机构实现。在无障碍情形下，理论上该机器人的末端可以实现空间任意的位姿，其精度主要表现在任务空间（某个笛卡尔空间）的定位精度。工业高性能的应用场合中，一般要求机器人能达到毫米级的定位精度。机器人的位置控制问题最终要驱动各个关节的电机运动，然后经过减速器等传动机构来实现，伺服电机的运动控制与定位精度是影响机器人末端定位精度的关键因素。
...
\begin{figure}[!htb]
	...
	\subfigure[第二轴电机系统的控制输入（电机电流）实际曲线]{\includegraphics[width=0.65\textwidth ]{ch5-semi-u.png}\label{fig.sim.elm.u}}
	%\subfigure[控制输入]{\includegraphics[width=0.5\textwidth ]{ch4-sim-elmu.png}\label{fig.sim.elm.u}}
...
\begin{figure}[!htb]
	...
	%\subfigure[控制输入]{\includegraphics[width=0.5\textwidth ]{ch4-sim-elmu.png}\label{fig.sim.elm.u}}
	\caption{半参数自适应控制算法的系统输出结果，关节角度跟踪误差的平均值为$0.0207$rad}
...
\begin{figure}[!htb]
	...
	\caption{第二轴电机系统的控制输入（电机电流）实际曲线}
	\label{fig.sim.pid.u}
\end{figure}
...
\item 半参数自适应估计的准确性导致最终的控制输入有比较好的变化曲线，对比图\ref{fig.sim.semi.u}和\ref{fig.sim.elm.u}容易得出。
\end{enumerate}
