...
\end{array}
\end{equation}
其中，$Q$、$\omega$和$\dot{\omega}$分别是电机输出的角位移（单位是rad）、角速度（单位是rad/s），角加速度（$\mathbf{rad/s^{2}}$）；$J_{0}$是折算到电机侧的所有转动惯量之和（包括传动机构和连杆耦合值等，单位是$\mathbf{kg\cdot m^{2}}$）；$B$是粘滞摩擦系数（单位是$\mathbf{N\cdot m\cdot s}$），$\tau_{f}$是Coulomb摩擦力矩（单位是$\mathbf{N\cdot m}$）；\tau_{l}是负载转矩；$K_{a}$和$K_{m}$分别是驱动放大器的导纳系数和电机的力矩常数；$i_{c}$是电机的绕组电流，$u_{c}$是控制电压。
...
额定电流（A）&1.4\\
最大电流（A）&4.3\\
转矩常数$K_{m}$（$\mathbfN{\cdot m\cdot A^{-1}}$）& 0.4571\\
