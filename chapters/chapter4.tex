...
i_k\triangle\mathop{\arg \min}\limits_{i<k}|y_k-y_i|
\end{equation}
...
\hat{z}_{k-1}=y_{k}-\phi_{k}^{\tau}\hat{\theta}_{k-1}-u_{k-1}
\end{equation}
...
& = (y_{k+1} - \phi_{k}^{\tau}\hat{\theta}_{k} - \breve{z}_{k} - u_{k} )\\
& + (\phi_{k}^{\tau}\hat{\theta}_{k} + \breve{z}_{k} + u_{k}  - y^{*}_{k+1})\\
