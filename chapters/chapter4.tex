...
则对于任意输入向量$\bm{x}_{j}$，第$l$个输出层神经元的输出为
\begin{equation}%
\label{eq:4.slfn}
\begin{split}%
o_{j,l}&=\sum_{i=1}^{N_{h}} \beta_{i} g(\bm{w}_{i},b_{i},\bm{x}_{j})\\
&=\sum_{i=1}^{N_{h}} \beta_{i} h(\bm{x}_{i})\\
&=\bm{h}(\bm{x}_{j})^{T}\cdot\bm{\beta}
\end{split}
\end{equation}
其中，$l=1,2,\dots,N_{o}$。这里$\bm{h}(\bm{x}_{j})$为隐含层的输出向量，$\bm{w}_{i}$和$b_{i}$分别是连接第$i$个隐层神经元与输入层的权重向量和偏置。
